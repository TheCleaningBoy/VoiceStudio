...
function watchMedia(el, label){
  ...
  el.addEventListener("error", () => {
    ...
    toast(`⚠️ Couldn't play ${label || "media"} (code ${e ? e.code : "?"}: ${e ? mediaErrName(e.code) : "unknown"}) — ${decodeURIComponent(src)} — app v1.3.1`, 9000);
  });
  return el;
}
...
function startRecording(stream, meterEl){
  ...
  // raw sample capture → AudioBuffer for sample-accurate mixing later
  const capSrc = ctx.createMediaStreamSource(stream);
  const proc = ctx.createScriptProcessor(4096, 1, 1);
  const mute = ctx.createGain(); mute.gain.value = 0;
  const capChunks = [];
  proc.onaudioprocess = e => { capChunks.push(new Float32Array(e.inputBuffer.getChannelData(0))); };
  capSrc.connect(proc); proc.connect(mute); mute.connect(ctx.destination);
  ...
  return {
    ...
    stop(){
      return new Promise(res => {
        rec.onstop = () => {
          ...
          // build the captured AudioBuffer
          let buffer = null;
          try{
            proc.disconnect(); mute.disconnect(); capSrc.disconnect();
            const total = capChunks.reduce((s,c) => s+c.length, 0);
            if(total > 0){
              buffer = ctx.createBuffer(1, total, ctx.sampleRate);
              const ch = buffer.getChannelData(0);
              let off = 0;
              for(const c of capChunks){ ch.set(c, off); off += c.length; }
            }
          }catch(e){}
          const blob = new Blob(chunks, {type: rec.mimeType || "audio/webm"});
          res({ blob, url: URL.createObjectURL(blob), buffer,
    ...
    cancel(){
      cancelAnimationFrame(raf);
      try{ src.disconnect(); }catch(e){}
      try{ proc.disconnect(); mute.disconnect(); capSrc.disconnect(); }catch(e){}
  ...
  if(s.kind === "pack" && s.pack.dubLines.length){
    ...
    function runPackDub(){
      makePackDubFlow({
        ...
        hearScene: document.getElementById("dub-hear").checked,
...
/** Render all of a take's lines into ONE wav blob — playback-proof. */
async function renderDubMix(take, cap){
  const ctx = getCtx();
  let jobs = take.lines.map((L,i) => {
    // each take is trimmed to the gap before the next line so lines never pile up
    const next = take.lines[i+1] ? take.lines[i+1].start : Infinity;
    let gap = next > L.start ? next - L.start : 3;
    if(cap && isFinite(cap)) gap = Math.min(gap, Math.max(0.5, cap + 0.5 - L.start));
    return {at: Math.max(0, L.start), buffer: null, url: L.url, cap: L.buffer || null, trim: Math.max(0.25, gap)};
  });
  // never place lines beyond the end of the scene
  let skipped = 0;
  if(cap && isFinite(cap)){
    const before = jobs.length;
    jobs = jobs.filter(j => j.at <= cap + 0.5);
    skipped = before - jobs.length;
  }
  await Promise.all(jobs.map(async j => {
    // 1) decode the recorded blob — the exact audio the user already hears in "Hear my take"
    try{
      const ab = await (await fetch(j.url)).arrayBuffer();
      j.buffer = await ctx.decodeAudioData(ab);
    }catch(e){}
    // 2) fall back to the raw captured samples
    if(!j.buffer && j.cap && bufferPeak(j.cap) >= 0.0005) j.buffer = j.cap;
  }));
  const ok = jobs.filter(j => j.buffer);
  if(!ok.length) return {ok:false, total: take.lines.length, mixed: 0, skipped};
  const end = Math.min(Math.max(...ok.map(j => j.at + j.buffer.duration)), (cap && isFinite(cap)) ? cap + 1 : Infinity);
  const sr = ctx.sampleRate;
  const off = new OfflineAudioContext(1, Math.ceil((end + 0.5)*sr), sr);
  for(const j of ok){
    const s = off.createBufferSource();
    s.buffer = j.buffer;
    s.connect(off.destination);
    s.start(j.at, 0, j.trim);
  }
  const rendered = await off.startRendering();
  return {ok:true, total: take.lines.length, mixed: ok.length, skipped,
          url: URL.createObjectURL(encodeWav(rendered)), duration: rendered.duration};
}
async function playPackDub(vid, pack, take){
  stopCurrentAudio();
  const ctx = getCtx();
  vid.muted = true; vid.currentTime = 0;

  // Backing track: plain audio started synchronously inside the click
  if(pack.backingUrl){
  ...
  const dbg = document.getElementById("dub-debug");
  const setDbg = t => { if(dbg) dbg.textContent = t; };
  setDbg("⏳ mixing your lines…");
  const cap = (vid.duration && isFinite(vid.duration)) ? vid.duration : Infinity;
  const mix = await renderDubMix(take, cap);

  if(!mix.ok){
    toast(`⚠️ Voice-over mix failed — 0 of ${mix.total} lines could be loaded (app v1.3.1)`, 9000);
    setDbg(`❌ mix FAILED: 0/${mix.total} lines loaded · audio engine: ${ctx.state}`);
  } else {
    let base = `✅ mix OK: ${mix.mixed}/${mix.total} lines → ${mix.duration.toFixed(1)}s track`;
    if(mix.skipped) base += ` · ⚠️ ${mix.skipped} line(s) skipped (past the end of the scene)`;
    const a = new Audio(mix.url);
    currentAudios.push(a);
    a.onplaying = () => setDbg(base + " · 🔊 PLAYING");
    a.onpause   = () => { if(!a.ended) setDbg(base + " · ⏸ PAUSED/BLOCKED — not playing!"); };
    a.onerror   = () => setDbg(base + " · ❌ player error " + (a.error ? a.error.code : "?"));
    const dl = document.getElementById("pk-dlmix");
    if(dl){ dl.href = mix.url; dl.setAttribute("download", "my-dub-mix.wav"); dl.classList.remove("hidden"); }
    setDbg(base + " · starting…");
    const tryPlay = () => a.play().catch(e => {
      setDbg(base + ` · ❌ BLOCKED: ${e.name || e.message}`);
      toast(`⚠️ Playback blocked: ${e.name || e.message} (app v1.3.1)`, 8000);
    });
    tryPlay();
    setTimeout(() => {
      if(a.paused && !a.ended){
        setDbg(base + " · ⚠️ silent after 0.7s — retrying play()");
        tryPlay();
      }
    }, 700);
    if(mix.mixed < mix.total) toast(`⚠️ Only ${mix.mixed} of ${mix.total} lines could be mixed (app v1.3.1)`, 6000);
  }
...
/** Export the full dubbed scene as a video file (video + mixed voice-over). */
async function exportDubVideo(vid, pack, take, dbgEl){
  const setDbg = t => { if(dbgEl) dbgEl.textContent = t; };
  if(!vid.captureStream){ toast("⚠️ Video export isn't supported in this build (app v1.3.1)", 8000); return null; }
  const ctx = getCtx();

  setDbg("🎥 Preparing export…");
  const cap = (vid.duration && isFinite(vid.duration)) ? vid.duration : Infinity;
  const mix = await renderDubMix(take, cap);
  if(!mix.ok){ toast("⚠️ No recorded lines to export (app v1.3.1)", 6000); return null; }
  const mixBuf = await ctx.decodeAudioData(await (await fetch(mix.url)).arrayBuffer());

  // audio graph: voice mix (+ backing track) → recorded stream, and to the speakers
  const dest = ctx.createMediaStreamDestination();
  const mixSrc = ctx.createBufferSource(); mixSrc.buffer = mixBuf;
  mixSrc.connect(dest); mixSrc.connect(ctx.destination);
  let backingEl = null;
  if(pack.backingUrl){
    backingEl = new Audio(pack.backingUrl); backingEl.volume = 0.55;
    const bsrc = ctx.createMediaElementSource(backingEl);
    bsrc.connect(dest); bsrc.connect(ctx.destination);
  }

  // video capture + combined recorder
  ...
  const rec = new MediaRecorder(combined, mime
    ? {mimeType: mime, videoBitsPerSecond: 6000000, audioBitsPerSecond: 192000}
    : undefined);
  const chunks = [];
  rec.ondataavailable = e => { if(e.data && e.data.size) chunks.push(e.data); };
  const stopped = new Promise(res => rec.onstop = res);

  const dur = (vid.duration && isFinite(vid.duration)) ? vid.duration : mixBuf.duration;
  const progInt = setInterval(() => {
    const pct = Math.min(100, (vid.currentTime / dur) * 100);
    setDbg(`🎥 Exporting your dub… ${pct.toFixed(0)}% — runs in real time, keep this window open`);
  }, 250);

  vid.muted = true; vid.currentTime = 0;
  rec.start(250);
  mixSrc.start();
  if(backingEl) backingEl.play().catch(()=>{});
  await vid.play();
  await new Promise(res => { vid.onended = res; setTimeout(res, (dur + 2) * 1000); });
  clearInterval(progInt);
  setDbg("🎥 Finishing export…");
  rec.stop();
  await stopped;
  try{ mixSrc.stop(); }catch(e){}
  if(backingEl) backingEl.pause();
...
function makePackDubFlow(cfg){
  ...
  async function boot(){
    ...
    cfg.videoDur = dur;
    if(!cfg.lines) cfg.lines = cfg.pack.dubLines.slice();
    const N = cfg.lines.length;
    const parsed = cfg.lines.map(l => l.time);
    let s = null;
    if(parsed.length && parsed.every(t => typeof t === "number" && !isNaN(t))){
      s = parsed.slice();
      const maxS = Math.max(...s);
      // sanity-check against the real scene length:
      // some packs store milliseconds (or stranger units) instead of seconds
      if(dur > 0 && maxS > dur + 2){
        if(maxS/1000 <= dur + 2){ s = s.map(t => t/1000); } // ms → s
        else s = null; // unusable → fall back to even spread below
      }
    }
    if(!s){
      const total = dur || N*4;
      s = Array.from({length:N}, (_,i) => +(i*(total/N)).toFixed(2));
    }
    // perform lines in SCENE order (by timestamp), not filename order
    const order = s.map((t,i) => i).sort((a,b) => s[a] - s[b]);
    starts = order.map(i => s[i]);
    cfg.lines = order.map(i => cfg.lines[i]);
    cfg.total = dur || (Math.max(...starts) + 8);
  ...
  function lineWindow(k){
    const vd = cfg.videoDur || Infinity;
    let end = k < cfg.lines.length-1 ? starts[k+1] : Math.min(cfg.total || starts[k]+8, starts[k]+8);
    end = Math.min(end, vd); // never record past the end of the scene
    // at least 4s to actually speak; longer takes get trimmed to fit the gap in the mix
    return clamp(Math.max(end - starts[k], 4), 1, 15);
  }
  ...
  function introPhase(){
    ...
    cfg.phaseEl.innerHTML = `
      ...
      <p class="muted">${cfg.lines.length} lines to record, one at a time — your takes get stitched back into the scene at the right timestamps. Retakes allowed per line.</p>
      <p class="muted small">Scene: ${cfg.videoDur ? cfg.videoDur.toFixed(1) : "?"}s · lines placed from ${starts[0].toFixed(1)}s to ${starts[starts.length-1].toFixed(1)}s</p>
      <video id="${cfg.videoTag}" class="game-vid" playsinline muted></video>
      <div class="row">
  ...
  function linePhase(k, recorded){
    const p = cfg.players[idx];
    setHud(p, `Line ${k+1}/${cfg.lines.length}`);
    const line = cfg.lines[k];
    cfg.phaseEl.innerHTML = `
      <div class="phase-title">Line ${k+1} of ${cfg.lines.length}</div>
      <div class="line-card">
  ...
  async function recordLine(k, recorded){
    const withScene = !!cfg.hearScene;
    const startAt = Math.max(0, starts[k]);
    const lead = withScene ? Math.min(3, startAt) : 0;
    // Pre-roll: begin capturing this many seconds BEFORE the line's timestamp and
    // shift the take's placement earlier by the same amount. This absorbs mic/
    // recorder start latency so the start of the performer's voice is never
    // clipped or pushed off the line's true moment.
    const PRE = withScene ? 0.4 : 0;

    // subtitle-style caption of the line being performed
    const line = cfg.lines[k];
    const capText = line.text ? `“${escapeHtml(line.text)}”` : `🎵 ${escapeHtml(line.name)}`;
    const capChar = line.character ? `<span class="cap-char">${escapeHtml(line.character)}:</span> ` : "";
    const captionHtml = withScene
      ? `<div class="rec-wrap">
           <video id="${cfg.videoTag}" class="game-vid" playsinline></video>
           <div class="caption-overlay" style="display:block">${capChar}${capText}</div>
         </div>`
      : `<div class="rec-caption">${capChar}${capText}</div>`;

    cfg.phaseEl.innerHTML = `
      ${captionHtml}
      <div class="phase-title" id="pk-rectitle">${withScene ? "🎧 Get ready… the scene is playing" : "SAY YOUR LINE!"}</div>
      <div class="meter"><div class="meter-fill" id="pk-meter"></div></div>
      <div class="rec-timer" id="pk-timer">${withScene ? "🎧 scene starting…" : "0.0s"}</div>
      <div class="row"><button class="btn bad" id="pk-stop">■ Finish early</button></div>`;

    // Start the scene SYNCHRONOUSLY inside the user's click so the browser
    // can never block it — the scene's lead-in replaces the countdown.
    let vid = null;
    if(withScene){
      vid = document.getElementById(cfg.videoTag); watchMedia(vid, "the scene");
      vid.src = cfg.pack.videoUrl;
      vid.muted = false; vid.volume = 1;
      const seekTo = startAt - lead;
      vid.currentTime = seekTo;
      vid.addEventListener("loadedmetadata", () => { try{ vid.currentTime = seekTo; }catch(e){} }, {once:true});
      const p = vid.play();
      if(p && p.then) p.catch(e => {
        toast(`⚠️ Scene playback was blocked (${e.name || "error"}) — try the toggle off/on (app v1.3.1)`, 8000);
        vid.muted = true; vid.play().catch(()=>{}); // keep the video moving silently
      });
    }

    let stream;
    try{ stream = await ensureMic(); }catch(e){ toast("🎤 " + e.message); if(vid){ try{ vid.pause(); }catch(x){} } return; }
    stopCurrentAudio();

    if(withScene){
      if(lead > 0){
        // let the performer hear the scene approach the line, then record exactly at the timestamp
        await new Promise(res => {
          let settled = false;
          const done2 = () => { if(!settled){ settled = true; res(); } };
          const poll = () => {
            if(vid.currentTime >= startAt - PRE || vid.ended || vid.error) done2();
            else requestAnimationFrame(poll);
          };
          requestAnimationFrame(poll);
          setTimeout(done2, (lead + 2) * 1000); // safety
        });
      }
      const tt = $("#pk-rectitle");
      if(tt) tt.innerHTML = '<span class="rec-dot"></span>SAY YOUR LINE — talk over the scene!';
    } else {
    ...
    const finish = async () => {
      ...
      const take = await session.stop();
      recorded[k] = {start: starts[k] - PRE, url: take.url, buffer: take.buffer || null};
  ...
  function lineReview(k, recorded){
    const isLast = k >= cfg.lines.length-1;
    cfg.phaseEl.innerHTML = `
  ...
  function assemblePhase(recorded){
    ...
    cfg.phaseEl.innerHTML = `
      ...
      <video id="${cfg.videoTag}" class="game-vid" playsinline muted></video>
      <div id="dub-debug" class="muted small" style="margin-top:10px"></div>
      <div class="row">
        <button class="btn primary" id="pk-playfull">▶ Play assembled dub</button>
        <a class="btn ghost hidden" id="pk-dlmix">⬇️ Download my dub mix (.wav)</a>
        <button class="btn ghost" id="pk-export">🎥 Export dub as video (.webm)</button>
    ...
    $("#pk-export").onclick = async () => {
      const btn = $("#pk-export"); btn.disabled = true;
      stopCurrentAudio();
      try{
        const res = await exportDubVideo(vid, cfg.pack, take, $("#dub-debug"));
        if(res){
          const name = (cfg.pack.name + " - my dub.webm").replace(/[^\w\- .()\[\]]+/g, "_");
          const dbg = $("#dub-debug");
          dbg.innerHTML = `✅ Video export ready (${(res.size/1048576).toFixed(1)} MB) — <a class="btn small primary" id="pk-dlvid" style="margin-left:8px">⬇️ Download video</a>`;
          const dla = $("#pk-dlvid"); dla.href = res.url; dla.download = name;
          const auto = document.createElement("a");
          auto.href = res.url; auto.download = name; auto.click();
        }
      }catch(e){
        toast("⚠️ Export failed: " + e.message + " (app v1.3.1)", 8000);
      }
      btn.disabled = false;
    };
